...
namespace IdentityServer.Repositories
{
    public class ProviderUserManagementRepository : IUserManagementRepository
    {
        ...
        private static void ValidatePasswordStrength(string password)
        {
            ...
            {
                throw new ValidationException(string.Format("{0} is the minimum password length",
            ...
            if (provider.MinRequiredNonAlphanumericCharacters > 0)
            {
                ...
                if (num2 < provider.MinRequiredNonAlphanumericCharacters)
                {
                    throw new ValidationException(
                        string.Format("{0} is the minimum number of non-alphanumeric characters",
                            provider.MinRequiredNonAlphanumericCharacters));
                }
            }
            if (!string.IsNullOrWhiteSpace(provider.PasswordStrengthRegularExpression) &&
                !Regex.IsMatch(password, provider.PasswordStrengthRegularExpression))
            {
                throw new ValidationException(string.Format("Password does not match the regular expression {0}",
