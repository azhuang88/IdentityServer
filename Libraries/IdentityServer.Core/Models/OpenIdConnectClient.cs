...
namespace IdentityServer.Models
{
    public class OpenIdConnectClient : IValidatableObject
    {
        ...
        [Display(Name = "Flow",
            Description =
                "OAuth2 flow for the client -- either server-side client (code) or naitve/javascript client (implicit)."
            )]
        [UIHint("Enum")]
        ...
        [Display(Name = "Refresh Token Lifetime",
            Description = "Lifetime (in minutes) of refresh token issued to client. Only allowed for code flow clients."
            )]
        public int RefreshTokenLifetime { get; set; }

        [Display(Name = "Require Consent",
            Description = "For this client should user be prompted to grant consent to access the user's profile data.")
        ]
