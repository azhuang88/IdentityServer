...
namespace IdentityServer.Protocols.OAuth2
{
    public class OAuth2TokenController : ApiController
    {
        ...
        public HttpResponseMessage Post([FromBody] TokenRequest tokenRequest)
        {
            ...
            if (tokenRequest.Grant_Type.Equals(OAuth2Constants.GrantTypes.AuthorizationCode))
            {
                return ProcessAuthorizationCodeRequest(client, tokenRequest.Code, tokenType);
            }
            if (string.Equals(tokenRequest.Grant_Type, OAuth2Constants.GrantTypes.RefreshToken, StringComparison.Ordinal))
            {
