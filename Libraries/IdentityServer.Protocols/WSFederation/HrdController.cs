...
namespace IdentityServer.Protocols.WSFederation
{
    ...
    {
        private const string _cookieName = "hrdsignout";
        private const string _cookieNameIdp = "hrdidp";
        private const string _cookieNameRememberHrd = "hrdSelection";
        private const string _cookieContext = "idsrvcontext";
        private const string _cookieOAuthContext = "idsrvoauthcontext";
        ...
        public HrdController(IConfigurationRepository configurationRepository,
        ...
        public async Task<ActionResult> OAuthTokenCallback()
        {
            ...
            claims.Add(new Claim(Constants.Claims.IdentityProvider, ip.Name, ClaimValueTypes.String,
        ...
        private ActionResult ProcessSignInRequest(SignInRequestMessage message)
        {
            if (!string.IsNullOrWhiteSpace(message.HomeRealm))
            {
                ...
                if (idp != null)
                {
                    ...
                    if (idp.Type == IdentityProviderTypes.OAuth2)
                    {
        ...
        private ActionResult ProcessWSFedSignInResponse(SignInResponseMessage responseMessage, SecurityToken token)
        {
            ...
            principal.Identities.First().AddClaim(
                new Claim(Constants.Claims.IdentityProvider, issuerName, ClaimValueTypes.String,
                    Constants.InternalIssuer));
            ...
            return new WSFederationResult(wsFedResponse, ConfigurationRepository.WSFederation.RequireSslForReplyTo);
        }
        ...
        private ProviderType GetProviderTypeFromOAuthProfileTypes(OAuth2ProviderTypes type)
        {
            switch (type)
            {
                case OAuth2ProviderTypes.Facebook:
                ...
                case OAuth2ProviderTypes.Live:
                    return ProviderType.Live;
                case OAuth2ProviderTypes.Google:
                    return ProviderType.Google;
                case OAuth2ProviderTypes.LinkedIn:
                    return ProviderType.LinkedIn;
                default:
                    throw new Exception("Invalid OAuthProfileTypes");
            }
        }

        private IdentityProvider GetEnabledIdentityProvider(string homeRealm)
        {
        ...
        private IEnumerable<IdentityProvider> GetEnabledOAuthIdentityProviders()
        {
            return IdentityProviderRepository.GetAll().Where(
                x => x.Enabled && x.Type == IdentityProviderTypes.OAuth2);
        }

        private IEnumerable<IdentityProvider> GetVisibleIdentityProviders()
        {
            return IdentityProviderRepository.GetAll().Where(
        ...
        private string GetRememberHRDCookieValue()
        {
            if (Request.Cookies.AllKeys.Contains(_cookieNameRememberHrd))
            {
                ...
                if (idp == null)
                {
                    Tracing.Verbose("Past HRD selection from cookie not found in current HRD list. Past value was: " +
                                    realm);
