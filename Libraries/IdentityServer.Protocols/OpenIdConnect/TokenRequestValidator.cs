...
namespace IdentityServer.Protocols.OpenIdConnect
{
    internal class TokenRequestValidator
    {
        ...
        private void ValidateRefreshTokenGrant(ValidatedRequest validatedRequest, TokenRequest request)
        {
            ...
            {
                throw new TokenRequestValidationException(
                    string.Format("Client {0} is trying to request a token using a refresh token from {1}.",
                        validatedRequest.Client.ClientId, grant.ClientId),
                    OAuth2Constants.Errors.InvalidGrant);
            }
        }
            ...
            if (!grant.RedirectUri.Equals(request.Redirect_Uri))
            {
                throw new TokenRequestValidationException(
                    string.Format(
                        ...
                        validatedRequest.RedirectUri, grant.RedirectUri),
                    OAuth2Constants.Errors.InvalidRequest);
            }
        }
    }
