...
namespace IdentityServer.Protocols.FederationMetadata
{
    ...
    public class WSFederationMetadataGenerator
    {
        ...
        public string GenerateAzRelyingPartyMetadata()
        {
            var tokenServiceDescriptor = GetTokenServiceDescriptor(_endpoints.WSFederationAZ.AbsoluteUri);
            //var appServiceDescriptor = GetLdapApplicationServiceDescriptor();
        ...
        private string GenerateMetadata(params RoleDescriptor[] roles)
        {
            ...
            ser.WriteMetadata(
                XmlWriter.Create(new StringWriter(sb), new XmlWriterSettings {OmitXmlDeclaration = true}), entity);
            return sb.ToString();
        }
